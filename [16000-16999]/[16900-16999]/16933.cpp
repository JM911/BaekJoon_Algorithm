...
int main()
{
	...
	while(!q.empty())
	{
		...
		int curDist = distMap[curX][curY][curZ];
		q.pop();
		...
		for(int l=0; l<4; l++)
		{
			...
			{
				if(curZ >= K) continue;
				if(distMap[x][y][curZ+1] <= 0 || distMap[x][y][curZ+1] > curDist + 1)
				{
					distMap[x][y][curZ+1] = curDist + (curDist % 2 == 1 ? 1 : 2);
					q.push({x, y, curZ+1});
				}
			}
			else
			{
				if(distMap[x][y][curZ] <= 0 || distMap[x][y][curZ] > curDist + 1)
				{
					distMap[x][y][curZ] = curDist + 1;
					q.push({x, y, curZ});
				}
			}
		}
